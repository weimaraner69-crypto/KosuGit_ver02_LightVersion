...
- `.env.example` の機密キー（`TOKEN` / `SECRET` / `PASSWORD` / `PASSWD` / `API_KEY`）に非空値がある状態
...
### SEC-011: security-regression 失敗時の一次対応フロー

GitHub Actions の `security-regression` ジョブが失敗した場合は、PRへ自動投稿されるコメントと Step Summary を起点に、以下の順で対応する。

1. **初動確認（5分以内）**
    - PRコメントの run URL から失敗ログを確認する
    - 失敗したテスト名と失敗理由（仕様差分 / 実装不備 / 一時要因）を記録する
2. **担当者アサイン**
    - 仕様変更に伴う失敗: 当該機能の実装担当者を primary assignee に設定する
    - 共通基盤（認証・CSRF・セキュリティ設定）に跨る失敗: セキュリティ担当を reviewer として追加する
3. **再実行判断**
    - 一時要因（CI環境の瞬断等）が明確な場合のみ、再実行は1回まで許可する
    - 同一失敗が再発した場合は再実行を止め、修正コミットを作成してから再検証する
4. **クローズ条件**
    - `security-regression` が成功していること
    - PRコメントまたはPR本文に、原因・対応内容・再発防止メモを残していること
