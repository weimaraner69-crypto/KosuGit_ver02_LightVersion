...
- SEC-011 セキュリティテスト整備: 完了（現行構成）
	...
	- 方針: 当面は現行のシンプル運用（起票者 + リポジトリオーナー候補）を維持し、高度な候補自動選出は運用課題が顕在化するまで見送る
	- 反映: `ci/policy_check.py` で `docs/sec-triage-spec-freeze.md` の必須セクション/参照先/版表記を検証し、凍結仕様との乖離を検知
	- 反映: `docs/sec-triage-spec-freeze.md` 変更時に、変更履歴の版番号増分（例: `v1.0`→`v1.1`）を `ci/policy_check.py` で必須化
